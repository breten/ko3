...
class Ko {
	constructor (options = {}) {
		const conf = {
			...
			dependsDist   : false,
			docsDist      : 'mdocs',
			docsIgnore    : ['node_modules', 'mdocs', '_js', 'dist', 'ko.config.js'],
			prjName       : '',
			useModLoc     : false,
		...
		const prjKoConfPath = path.resolve('.',this.options.pagesDir, this.options.prjName, 'ko.config.js')
		if(fs.existsSync(prjKoConfPath)) {
			let _options = require(prjKoConfPath)
			this.options = _.defaultsDeep(_options, this.options)
		}
