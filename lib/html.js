  ...
  let result = compile({
    'Sinclude': function(vmId, includeType) {
      ...
      if (vmCache[targetPath] && vmCache[targetPath][vmId] && vmCache[targetPath][vmId]['jsPath']) {
          ...
          const dir = jsPath.replace(ko.options.distDir + '/' + ko.options.pagesDir, '')
...
function parseVm(content, filePath) {
  ...
  if (_vms.length > 0) {
    ...
    _vms.forEach((vm, i) => {
      ...
      const _distJsPath = path.join(ko.options.distDir, _dirPath, ko.options.jsDir, (_.camelCase(vm.pid + '-' + vm.vid) + (vm.include ? '.' + vm.include : '') + '.js'))
...
function output() {
  return new Promise((resolve, reject) => {
    ...
    files.forEach(function(file, i) {
      ...
      const distTarPath = path.resolve(this.options.distDir, _dirPath, 'html', _basename)
