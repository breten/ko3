...
function output() {
  return new Promise((resolve, reject) => {
    ...
    const dependsDist = path.join(this.options.distDir, this.options.prjName, 'depends.json')
    let depends = JSON.parse(fs.readFileSync(path.resolve(dependsDist), 'utf-8'))
    depends.pages = stats  
    fs.writeFileSync(dependsDist, beautify(JSON.stringify(depends), {
      indent_size: 2
    }), 'utf-8')
