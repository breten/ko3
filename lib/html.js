  ...
  let result = compile({
    'Sinclude': function(vmId, includeType) {
      ...
      if (vmCache[targetPath] && vmCache[targetPath][vmId] && vmCache[targetPath][vmId]['jsPath']) {
        ...
        dir = jsPath.replace(ko.options.distDir, '')
        dirForDev = jsPath.replace(ko.options.distDir, '')
      }else {
        jsPath = path.join(ko.options.distDir, path.dirname(targetPath).replace(ko.options.pagesDir, ''), 'js', vmId + '.js')
        dir = jsPath.replace(ko.options.distDir, '')
        dirForDev = jsPath.replace(ko.options.distDir, '')
      ...
      if(dev) {
        if(dev.outputFileSystem.existsSync(path.resolve(jsPath))){
          ...
          throw new Error(`${jsPath} not found`)
        }
      }else{
