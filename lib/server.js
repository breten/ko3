...
class Serve {
	constructor (ko) {
		...
		this.wpConfig = mergeWpConf(wpConfigBase(ko), ko.options.webpack({
			isDev: true
		}) || {})
		this.htmlChanged = 0
	}
	...
	start () {
	  ...
	  this.wpConfig.entry = _.defaultsDeep(this.getEntries(), this.wpConfig.entry)
	  ...
	  const ko = this.ko
        ...
        const _dirPath = path.dirname(file)
	      ...
	      return Promise.resolve(result)
	      	...
	      	.then((result) => {
		        ...
		        res.write(result)
		        res.end('<script src="//'+ devOptions.host +':'+ devOptions.port +'/webpack-dev-server.js"></script>')
	      	})
	      	.catch((err) => {
	      		console.log(err)
	      	})
		...
		this.server.listen(devOptions.port, devOptions.host, function(err) {
			if(err) throw err
			console.log(`Serving at: ${devOptions.https ? 'https' : 'http'}:\/\/${devOptions.host}:${devOptions.port}`)
