...
function deploy () {
	...
	function sendDocs() {
		if(!koOptions.dependsDist && !koOptions.docsDist) {
		...
		var docsPath = [path.resolve(koOptions.rootDir, koOptions.dependsDist, '**/*.json'),
			path.resolve(koOptions.rootDir, koOptions.docsDist, '**/*.json')]
		return new Promise(function(resolve, reject){
			vfs.src(docsPath, {
