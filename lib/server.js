...
class Serve {
	...
	start () {
	  ...
	  wpConfig.entry = _.defaultsDeep(getEntries([entryGlob, entryGlobPlus], this.options.pagesDir), getEntries([entryHtmlGlob], this.options.pagesDir, this.options.tmpDir),wpConfig.entry)
	  ...
	  const ko = this.ko
        ...
        const _dirPath = path.dirname(file)
	      ...
	      const _content = compiler.outputFileSystem.readFileSync(path.resolve(distHtmlTarPath), 'utf-8')
