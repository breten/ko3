...
function deploy (opts) {
	...
	function sendDocs() {
		if(!koOptions.dependsDist && !koOptions.docsDist || !koOptions.docSftp.remotePath) {
			return Promise.resolve()
		}
		var docSSH = new ssh({
		  sshConfig: koOptions.docSftp
		});

		var docsPath = []

		koOptions.dependsDist && docsPath.push(path.resolve(koOptions.rootDir, koOptions.dependsDist, '**/*.json'))
		koOptions.docsDist &&	docsPath.push(path.resolve(koOptions.rootDir, koOptions.docsDist, '**/*.json'))
		return new Promise(function(resolve, reject){
			vfs.src(docsPath, {
				base: path.resolve(koOptions.rootDir)
			})
			.pipe(docSSH.dest(path.join(koOptions.docSftp.remotePath)))
			.on('finish', function () {
				docSSH.close();
				resolve();
			})
	...
	opts.doc && (koOptions.dependsDist || koOptions.docsDist) && arr.push(sendDocs);
	arr.push(sshEnd);
